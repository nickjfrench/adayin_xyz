...
declare global {
  interface Window {
    /** Set by `initDestBar` so the hero dropdown can drive the destination bar. */
    __destBarSelect?: (slug: string) => void
  }
}
 ...
 */
export function initDestBar(onSelect?: (slug: string, city?: string) => void) {
  function select(slug: string) {
    setActiveChip(slug)
    const chip =
      slug === 'all' ? null : document.querySelector<HTMLElement>(`.dest-jump[data-dest="${slug}"]`)
