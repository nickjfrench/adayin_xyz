<script>
  ...
  // The modal mirrors `stop`: opening and closing both flow through it (Modal
  // reports a close via `onclose`, and `close()` clears `stop`).
  const show = $derived(stop != null)
...
{#snippet body()}
  {#if stop.longDesc}
    <div class="pt-inline text-sm text-sea-600 italic">
      <PortableText value={stop.longDesc} components={ptComponents} />
    </div>
  {/if}

  {#if labeledFeatures.length > 0}
    <div class="mt-4 flex flex-col gap-1.5">
      <p class="text-xs font-semibold tracking-wide text-sand-500 uppercase">Spots &amp; options</p>
      <ul class="flex flex-col gap-1">
        {#each labeledFeatures as f (f._key)}
  ...
  {#if stop.callouts?.length > 0}
    <div class="mt-4 flex flex-col gap-2">
      {#each stop.callouts as callout (callout._key)}
        <div class="pt-inline flex items-start gap-2 text-sm text-sea-700">
          {#if callout.kind?.icon?.svg}
            <SanityIcon
              svg={callout.kind.icon.svg}
              class="mt-0.5 h-3.5 w-3.5 shrink-0 {callout.kind?.labelColor ?? 'text-sand-500'}"
            />
          {:else}
            <span
              class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full {callout.kind?.labelColor ??
                'bg-sand-400'}"
            ></span>
          {/if}
  ...
  {#if stop}
    <div class="-m-6">
      ...
      <div class="relative">
        ...
        {/if}
        <div
          class="absolute inset-0 bg-gradient-to-t from-sea-900/80 via-sea-900/30 to-sea-900/10"
        ></div>
        ...
        <div class="absolute inset-x-0 bottom-0 z-10 px-6 pb-8">
          {#if isStart}
            <span
              class="inline-flex items-center gap-2 rounded-full bg-green-500/95 px-3 py-1 font-display text-xs font-semibold tracking-wide text-white uppercase ring-1 ring-white/20"
            >
              Start location
            </span>
          {:else if isEnd}
            <span
              class="inline-flex items-center gap-2 rounded-full bg-sand-400/95 px-3 py-1 font-display text-xs font-semibold tracking-wide text-white uppercase ring-1 ring-white/20"
            >
              End location
            </span>
          {:else if isTravel && stop.travelType?.label}
            <span
              class="inline-flex items-center gap-2 rounded-full bg-sea-500/95 px-3 py-1 font-display text-xs font-semibold tracking-wide text-white uppercase ring-1 ring-white/20"
            >
              {#if stop.travelType.icon?.svg}
                <SanityIcon svg={stop.travelType.icon.svg} class="h-3 w-3" />
              {/if}
          ...
          {:else if stop.stopType?.label}
            ...
            >
              {#if stop.stopType.icon?.svg}
                <SanityIcon svg={stop.stopType.icon.svg} class="h-3 w-3" />
              {/if}
              {stop.stopType.label}
            </span>
          {:else}
          ...
          {#if stop.title}
            <h2
              class="mt-3 font-display text-2xl leading-snug font-semibold text-white drop-shadow-md"
            >
              {stop.title}
            </h2>
          {/if}
      ...
      {#if hasMeta}
        <div
          class="relative z-10 mx-4 -mt-5 flex flex-wrap items-center gap-x-3 gap-y-1.5 rounded-xl bg-white/90 px-5 py-3 text-sm font-medium text-sea-700 shadow-lg backdrop-blur-sm"
        >
          {#if stop.time}
            <span class="inline-flex items-center gap-1.5 text-sea-600">
              <svg
                class="h-3.5 w-3.5 text-sand-500"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
          ...
          {#if stop.cost != null && stop.cost > 0}
            ...
            <span class="inline-flex items-center gap-1 text-sea-600" title="Per person">
              {#if stop.currency?.icon?.svg}
                <SanityIcon svg={stop.currency.icon.svg} class="h-3.5 w-3.5 text-sand-500" />
              {/if}
